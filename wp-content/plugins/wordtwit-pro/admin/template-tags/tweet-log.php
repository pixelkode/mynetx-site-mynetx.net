...
function wordtwit_get_tweet_log_total_entries( $filter = false ) {
	...
	switch( $cur_filter ) {
		case 'all':
			$result = $wpdb->get_row( "SELECT count(*) AS c FROM " . $wpdb->posts . " WHERE post_type = 'tweet' AND post_status IN ('future', 'publish')" );
			break;	
		case 'publish':
			$result = $wpdb->get_row( "SELECT count(*) AS c FROM " . $wpdb->posts . " WHERE post_type = 'tweet' AND post_status = 'publish' AND post_content <> 'error'" );
			break;
		case 'future':
			$result = $wpdb->get_row( "SELECT count(*) AS c FROM " . $wpdb->posts . " WHERE post_type = 'tweet' AND post_status = 'future'" );
			break;
		case 'errors':
			$result = $wpdb->get_row( "SELECT count(*) AS c FROM " . $wpdb->posts . " WHERE post_type = 'tweet' AND post_status = 'publish' AND post_content = 'error'" );
			break;			
	...
	if ( $wordtwit_tweet_log_entry && $wordtwit_tweet_log_entry->ID ) {
		...
		if ( $associated_post_id ) {
			...
			if ( $tweet_info ) {
				...
				if ( !$tweet_info->manual && $settings->url_shortener != 'wordpress' ) {
					return true;
				}
			}
		}
