/* The WordTwit Pro Post Widget JS */
...
function doWordTwitWidget() {
	...
	jQuery( 'p.wt-tweet-text' ).live( 'click', function(){
		...
		wtEditingTweet = tweetText;
		jQuery( this ).replaceWith( '<textarea id="wt-tweet-textarea">'+tweetText+'</textarea>' );
...
var wordTwitDraftText = jQuery( '#post-status-info td.autosave-info #autosave' ).html();
function wtDraftSaveChecker() {
	var draftText = jQuery( '#post-status-info td.autosave-info #autosave' ).html();
	if ( draftText != wordTwitDraftText ) {
		wordTwitDraftText = draftText;
		wtSavePostBoxData();
	//	console.log( 'Draft saved, updating WordTwit tweet.' );	
	} else {
	//	console.log( 'No WP draft changes.' );	
	}
}

var wtChecker = '';
function wtStartDraftCheck() {
	if ( wtChecker == '' && !jQuery( '#wordtwit-post-widget' ).hasClass( 'disabled' ) ) {
		wtChecker = window.setInterval( 'wtDraftSaveChecker()', 1000 * 3 );  // check for new draft saves every 3 seconds
	//	console.log( 'WordTwit title check interval set (3 seconds).' );
	} else {
	//	console.log( 'Did not start draft check interval.' );
	}
}
...
/* Fired when the tweet text div is clicked on for in-place editing */
function wtSwapTextarea() {
	var textArea =	jQuery( 'textarea#wt-tweet-textarea' );
	var tweetText = jQuery( textArea ).val();
	jQuery( textArea ).replaceWith( '<p class="wt-tweet-text">'+tweetText+'</p>' ).remove();
		
	if ( tweetText != wtEditingTweet ) {
...
function wtSavePostBoxData() {
	...
	if ( ajax_params[ 'tweet_times' ] > 1 ) {
		ajax_params[ 'tweet_sep_hour' ] = jQuery( '#wt-select-hours' ).val();
...
/* The WordTwit Pro Admin Ajax function */
