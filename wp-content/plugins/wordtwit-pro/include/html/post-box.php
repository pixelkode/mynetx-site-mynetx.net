...
<div id="wordtwit-post-widget"<?php if ( wordtwit_is_post_box_area_disabled() ) echo ' class="disabled"';?>>
			...
			<?php if ( wordtwit_post_is_enabled() ) { ?>
				...
				<?php if ( wordtwit_get_pending_tweet_count() && wordtwit_get_publish_tweet_count() ) { ?>
					<a href="admin.php?page=tweet_queue">
						<span class="pending">
							<?php echo sprintf( __( "(%d Tweeted, %d Pending)", "wordtwit-pro" ), wordtwit_get_publish_tweet_count(), wordtwit_get_pending_tweet_count() ); ?>
						</span>
					</a>
				<?php } else if ( wordtwit_get_pending_tweet_count() ) { ?>
					<a href="admin.php?page=tweet_queue">
						<span class="pending">
							<?php echo sprintf( __( "(%d Pending)", "wordtwit-pro" ), wordtwit_get_pending_tweet_count() ); ?>
						</span>
					</a>
				<?php } else if ( wordtwit_get_publish_tweet_count() ) { ?>
					<a href="admin.php?page=tweet_queue">
						<span class="pending">
							<?php echo sprintf( __( "(%d Tweeted)", "wordtwit-pro" ), wordtwit_get_publish_tweet_count() ); ?>
						</span>
					</a>
				<?php } ?> 
				</p>
	...
	<div id="disabled-wrapper" <?php if ( wordtwit_is_post_box_area_disabled() ) { echo "style='display:none'"; } ?>>		
		...
		<div id="wt-widget-bottom-wrap" <?php global $post; if ( !isset( $_COOKIE['wordtwit-pro-' . $post->ID] ) ) { echo "style='display:none'"; } ?>>		
			<!-- Accounts -->
			<div id="wt-accounts-box" class="wt-widget-bottom" <?php global $post; if ( $_COOKIE['wordtwit-pro-' . $post->ID] != 'wt-accounts' ) { echo "style='display:none'"; } ?>>
				...
				<?php if ( wordtwit_has_accounts() ) { ?>
					...
					<?php while ( wordtwit_has_accounts() ) { ?>
						...
						<span>
							...
							<span class="acct-type"><?php if ( wordtwit_is_account_global() ) echo 'shared '; else echo 'private'; ?></span>
						</span>
					</li>
					<?php } ?>
					</ul>
			...
			<!-- Tags -->
			<div id="wt-tags-box" class="wt-widget-bottom" <?php global $post; if ( $_COOKIE['wordtwit-pro-' . $post->ID] != 'wt-tags' ) { echo "style='display:none'"; } ?>>
				...
				<div class="wt-automatic">
					...
					<p>
						<input type="text" id="wt-add-hashtag" name="wt-add-hashtag" class="newtag" size="34" placeholder="<?php _e( "Add New Hashtag", "wordtwit-pro" ); ?>" />
			...
			<!-- schedule -->
			<div id="wt-schedule-box" class="wt-widget-bottom" <?php global $post; if ( $_COOKIE['wordtwit-pro-' . $post->ID] != 'wt-schedule' ) { echo "style='display:none'"; } ?>>
				...
				<div id="wt-sheduling">
					...
					<select id="wt-select-hours">
						<option value="0"<?php if ( wordtwit_post_get_tweet_seperated_hours() == 0 ) echo ' selected';?>>
							<?php echo sprintf( __( '%d hours', 'wordtwit-pro' ), 0 ); ?>
						</option>
						<option value="1"<?php if ( wordtwit_post_get_tweet_seperated_hours() == 1 ) echo ' selected';?>>
							<?php echo sprintf( __( '%d hour', 'wordtwit-pro' ), 1 ); ?>
						</option>
							<?php for( $i = 2; $i <= 6; $i++ ) { ?>
						<option value="<?php echo $i; ?>"<?php if ( wordtwit_post_get_tweet_seperated_hours() == $i ) echo ' selected';?>>
							<?php echo sprintf( __( '%d hours', 'wordtwit-pro' ), $i ); ?>
						</option>
						<?php } ?>
						<option value="8"<?php if ( wordtwit_post_get_tweet_seperated_hours() == 8 ) echo ' selected';?>>
							<?php echo sprintf( __( '%d hours', 'wordtwit-pro' ), 8 ); ?>
						</option>
						<option value="12"<?php if ( wordtwit_post_get_tweet_seperated_hours() == 12 ) echo ' selected';?>>
							<?php echo sprintf( __( '%d hours', 'wordtwit-pro' ), 12 ); ?>
						</option>
						<option value="24"<?php if ( wordtwit_post_get_tweet_seperated_hours() == 24 ) echo ' selected';?>>
							<?php echo sprintf( __( '%d hours', 'wordtwit-pro' ), 24 ); ?>
						</option>
					</select>
	...
	<?php if ( ( wordtwit_get_tweet_status() == WORDTWIT_TWEET_PUBLISHED ) && wordtwit_has_post_active_accounts() ) { ?>
		<div class="wt-button-wrap">
			<a class="button wt-tweet-now-button" href="<?php wordtwit_the_retweet_post_url(); ?>"><?php _e( "Retweet", "wordtwit-pro" ); ?></a>
		</div>		
	<?php } else if ( wordtwit_get_tweet_status() == WORDTWIT_TWEET_IS_OLD ) { ?>		
		<div class="wt-button-wrap">
