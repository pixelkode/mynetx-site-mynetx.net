...
define( 'BNC_WORDTWIT_API_VERSION', '1.4' );

//! The URL used to access the BNC API
define( 'BNC_WORDTWIT_API_URL', 'http://api.bravenewcode.com/v/' . BNC_WORDTWIT_API_VERSION );

//! The WordPress setting name for the token information 
define( 'BNC_WORDTWIT_TOKEN_INFO_SETTING', 'bncid_wordtwit_pro_token' );

define( 'BNC_WORDTWIT_AUTHENTICATION_ATTEMPTS', 2 );

define( 'BNC_WORDTWIT_API_TIMEOUT', 8 );

define( 'BNC_WORDTWIT_PRODUCT_NAME', 'wordtwit-pro' );

define( 'WORDTWIT_DEBUG_WARNING', 0 );
define( 'WORDTWIT_DEBUG_INFO', 1 );
define( 'WORDTWIT_DEBUG_ERROR', 2 );

class WordTwitBNCAPI {
	//! The user's BNCID
	var $bncid;
	...
	function WordTwitBNCAPI( $bncid, $license_key ) {
		$this->bncid = $bncid;
		...
		$token_info = get_option( BNC_WORDTWIT_TOKEN_INFO_SETTING, false );
		if ( $token_info ) {
	...
	function do_api_request( $method, $command, $params = array(), $do_auth = false ) {
		$url = BNC_WORDTWIT_API_URL . "/{$method}/{$command}/";
        ...
        $options = array( 'method' => 'POST', 'timeout' => BNC_WORDTWIT_API_TIMEOUT, 'body' => $body );
        $options['headers'] = array(
            'Content-Type' => 'application/x-www-form-urlencoded; charset=' . get_option('blog_charset'),
            'Content-Length' => strlen( $body ),
            'User-Agent' => 'WordPress/' . get_bloginfo("version") . '/WordTwit',
        ...
        if ( !is_wp_error( $raw_response ) ) {
        	...
        	if ( $raw_response['response']['code'] == 200 ) {
        		...
        		WORDTWIT_DEBUG( WORDTWIT_DEBUG_WARNING, "Unable to connect to server. Response code is " . $raw_response['response']['code'] );
        	}
        } 
	...
	function get_product_list() {
		if ( $this->internal_check_token() ) {
			...
			$params = array(
				'bncid' => $this->bncid,
				'product_name' => BNC_WORDTWIT_PRODUCT_NAME
			);
	...
	function get_product_version( $beta = false ) {
		if ( $this->internal_check_token() ) {
	...
	function user_remove_license( $site_name ) {
		if ( $this->internal_check_token() ) {
	...
	function authenticate() {
		WORDTWIT_DEBUG( WORDTWIT_DEBUG_ERROR, "Authenticating" );
		...
		if ( $nonce ) {
			...
			$params['product_name'] = BNC_WORDTWIT_PRODUCT_NAME;
			...
			if ( $auth_result ) {
				if ( $auth_result['status'] == 'ok' ) {
					...
					update_option( BNC_WORDTWIT_TOKEN_INFO_SETTING, $token_info );
	...
	function invalidate_all_tokens() {
		delete_option( BNC_WORDTWIT_TOKEN_INFO_SETTING );	
	}
	...
	function check_token() {
		...
		if ( $this->token ) {
			...
			WORDTWIT_DEBUG( WORDTWIT_DEBUG_ERROR, "Performing server token check" );
	...
	function try_multiple_authentications() {
		...
		while ( true ) {
			...
			if ( $attempts > BNC_WORDTWIT_AUTHENTICATION_ATTEMPTS ) {
				break;	
			}	
		}	
	...
	function internal_check_token() {
		...
		if ( time() > ( $this->token_expires - 15 ) ) {
			WORDTWIT_DEBUG( WORDTWIT_DEBUG_INFO, "Authenticating due to expiry" );
		...
		if ( $succeeded && !$this->token_checked ) {	
			...
			if ( ( time() - $this->last_token_check ) > 300 ) {	
				WORDTWIT_DEBUG( WORDTWIT_DEBUG_INFO, 'Forcing token check due to cache expiry' );
	...
	function get_license_reset_info() {
		if ( $this->internal_check_token() ) {		
			$params = array();
			$params['bncid'] = $this->bncid;
			$params['product_name'] = BNC_WORDTWIT_PRODUCT_NAME;
			
			WORDTWIT_DEBUG( WORDTWIT_DEBUG_INFO, "Getting license reset information" );
