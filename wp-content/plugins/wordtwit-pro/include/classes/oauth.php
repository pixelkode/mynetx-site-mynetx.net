...
class WordTwitOAuth {
	...
	function do_request( $url, $oauth_header, $body_params = '' ) {		
		...
		if ( function_exists( 'curl_init' ) && $this->can_use_curl ) {
			...
			WORDTWIT_DEBUG( '..using CURL transport' );
			...
			if ( wordtwit_is_debug_enabled() ) {
				...
				WORDTWIT_DEBUG( '..POST parameters are ' . $param_str );
			}
			...
			if ( !ini_get('safe_mode') ) {
				...
				WORDTWIT_DEBUG( '..CURLOPT_FOLLOWLOCATION is ON' );
			}
			...
			if ( ( $contents = curl_exec( $ch ) ) ) {
  				...
  				WORDTWIT_DEBUG( '..CURL returned a status code of ' . $response[ 'http_code' ] );
  				
  				$this->response_code = $response[ 'http_code' ];
	  			if ( $response['http_code'] == 200 ) {
	  				WORDTWIT_DEBUG( '..RESPONSE was ' . print_r( $contents, true ) );
	  				
	  				return $contents;
	  			} else {
	  				WORDTWIT_DEBUG( '..RESPONSE was ' . print_r( $response, true ) );
				...
				WORDTWIT_DEBUG( "..CURL didn't return any contents" );
  				curl_close( $ch );
			}
		} else {
			$request = new WP_Http;
	...
	function get_request_token() {
		...
		$params['oauth_callback'] = 'http://' . $_SERVER['HTTP_HOST'] . add_query_arg( 'wordtwit_pro_oauth', 1 );
	...
	function update_status( $token, $token_secret, $status ) {
		...
		if ( $result ) {
			$new_params = wordtwit_pro_parsexml( $result );
			return true;
		} else {
			return false;	
		}	
	...
	function setup() {
		eval( base64_decode( 'ZGVmaW5lKCAnV09SRFRXSVRfT0FVVEhfQ09OU1VNRVJfU0VDUkVUJywgJ2tGM0pLaHdrUTFPMnlsbDVWU0hDaTZXekNWVU1TeUxKYzRBY1N3c0JvWEEnICk7' ) );
			
		$this->consumer_key = WORDTWIT_OAUTH_CONSUMER_KEY;	
		$this->consumer_secret = WORDTWIT_OAUTH_CONSUMER_SECRET;
	}
}