...
<table class="form-table">
			...
			<?php
			   	...
			   	if ( isset( $wpAppbox_MicrosoftPrivateAffiliateProgramm ) ):
					foreach ( $wpAppbox_MicrosoftPrivateAffiliateProgramm as $country => $programID ) {
						echo( "<option class=\"level-0\" value=\"" . $programID . "\" " . selected( get_option('wpAppbox_affiliateMicrosoftProgram'), $programID ) . ">" . esc_html__( $country, 'wp-appbox' ) . " (ID: " . $programID . ")</option>" );
					}
				endif;
			?>
			</select>
	...
	<tr valign="top" class="affiliateMicrosoft" <?php if ( get_option('wpAppbox_affiliateMicrosoftDev') ) echo( ' style="display:none;"' ); ?>>
		...
		<td>	
			<input type="text" pattern=".{0}|.{11}" title="<?php esc_html_e('Rakuten ID must be an 11-digit code.', 'wp-appbox'); ?>" name="wpAppbox_affiliateMicrosoftID" id="wpAppbox_affiliateMicrosoftID" value="<?php echo( get_option('wpAppbox_affiliateMicrosoftID') ); ?>" />
		</td>
	</tr>
