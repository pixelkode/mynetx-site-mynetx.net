...
<div id="comments">
    <div id="cmtswitcher">
        ...
        <a id="commenttab" class="curtab" href="javascript:void(0);">
            <?php _e('Comments', 'philna');echo ' (';?><span
                id="allcmnub"><?php echo count($comments) - count($trackbacks);?></span>)</a>
        ...
        <span class="addcomment"><a
                href="#respond"><?php _e('Leave a comment', 'philna'); ?></a></span>
        <?php endif; ?>
        <?php if (pings_open()) : ?>
        <span class="addtrackback"><a
                href="<?php trackback_url(); ?>"><?php _e('Trackback', 'philna'); ?></a></span>
        <?php endif; ?>
                ...
                if (!$comment->comment_type == 'pingback' && !$comment->comment_type == 'trackback')
                {
                    ...
                    else
                    {
                        ...
                        <div class="comment-meta">
                            ...
                            | #<span
                                class="cmnub"><?php printf('%1$s', ++$count); ?></span>
                            ...
                            <?php if (function_exists('get_avatar') && get_option('show_avatars'))
                        {
                            ...
                            <span class="fn"><a
                                    ...
                                    href="<?php comment_author_url() ?>"><?php comment_author(); ?></a></span>
                            <span class="reply" style="visibility:hidden;"><a
                                    href="#commentform" title=""
                                    onclick="document.getElementById('comment').value += '&lt;a href=&quot;#comment-<?php comment_ID() ?>&quot;&gt;@<?php comment_author();?> &lt;/a&gt;\n'"><img
                                    src="<?php bloginfo('template_url'); ?>/img/reply.gif"
                                    alt="Notify" /></a></span>
                            <?php else : ?>
                            <span class="fn"
                                  id="commentauthor-<?php comment_ID() ?>"><?php comment_author(); ?></span>
                            <span class="reply" style="display:none;"><a
                                    href="#commentform" title=""
                                    onclick="document.getElementById('comment').value += '&lt;a href=&quot;#comment-<?php comment_ID() ?>&quot;&gt;@<?php comment_author();?> &lt;/a&gt;\n'"><img
                                    src="<?php bloginfo('template_url'); ?>/img/reply.gif"
                                    alt="Notify" /></a></span>
                        ...
                        <p>
                            <small class="waiting">Your comment is awaiting
                                moderation.
                            </small>
                        </p>
                        <?php endif; ?>
            ...
            <?php foreach ($trackbacks as $comment) : ?>
                <li class="trackback">
                    ...
                    <div class="date">
                        <? printf(__('%1$s at %2$s', 'philna'), get_comment_time(__('F jS, Y', 'philna')), get_comment_time(__('H:i', 'philna'))); ?>
                        | <a
                            href="#comment-<?php comment_ID() ?>"><?php printf('#%1$s', ++$count); ?></a>
                    </div>
                    <div class="fixed"></div>
                    <div class="title">
...
<div id="respond">
    <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php"
        ...
        <div class="row"><?php _e('Logged in as', 'philna'); ?> <a
                href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php"><strong><?php echo $user_identity; ?></strong></a>.
            <a href="<?php echo $logout_link; ?>"
               title="<?php _e('Log out of this account', 'philna'); ?>"><?php _e('Logout &raquo;', 'philna'); ?></a>
        </div>
        ...
        <?php if ($comment_author != "") : ?>
            ...
            <div class="row welcome">
                ...
                <span id="show_author_info"><a
                        href="javascript:void(0);"><?php _e('Change &raquo;', 'philna'); ?></a></span>
            </div>
            <?php endif; ?>

        <div id="author_info"<?php if ($comment_author != "")
                {
				...
				<span class="row">
					<input type="text" name="author" id="author"
                           class="textfield"
                           value="<?php echo $comment_author; ?>" size="24"
                           tabindex="1" />
					<label for="author"
                           class="small"><?php _e('Name', 'philna'); ?> <?php if ($req)
                        _e('(required)', 'philna'); ?></label>
				</span>
				<span class="row">
					<input type="text" name="email" id="email" class="textfield"
                           ...
                           size="24" tabindex="2" />
					<label for="email"
                           class="small"><?php _e('E-Mail (will not be published)', 'philna');?> <?php if ($req)
                        _e('(required)', 'philna'); ?></label>
				</span>
				<span class="row">
					<input type="text" name="url" id="url" class="textfield"
                           value="<?php echo $comment_author_url; ?>" size="24"
                           tabindex="3" />
					<label for="url"
                           class="small"><?php _e('Website', 'philna'); ?></label>
				</span>
        </div>
        <?php endif; ?>
	...
	<script type="text/javascript">edToolbar('comment');</script>-->
        <div class="row cmarea">
            ...
            <textarea name="comment" id="comment" tabindex="4" rows="8"
                      cols="50"></textarea>

            <div id="commentload" class="center" style="display:none;">
                <img src="<?php bloginfo('template_url'); ?>/img/ajaxload.gif"
                     alt="loading" /> <?php _e('Submitting Comment, Wait me a second...', 'philna')?>
            </div>
        </div>
        <div id="submitbox">
            <a class="feed"
               href="<?php bloginfo('comments_rss2_url'); ?>"><?php _e('Comments feed', 'philna'); ?></a>
            <script type="text/javascript">
                /* <![CDATA[ */
                //ctrl+enter
            ...
            <input name="submit" type="submit" id="submit" class="button"
                   tabindex="5"
                   value="<?php _e('Submit Comment', 'philna'); ?>" />
