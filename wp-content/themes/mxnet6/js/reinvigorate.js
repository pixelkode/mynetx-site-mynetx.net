...
function re_tt_platform() // Text Pattern
{
    re_add_event(window, "onload", function()
    {
        ...
        re_add_event(f["submit"], "onclick", function()
        {
            re_make_comment(f.name.value, f.email.value, f.web.value, msg);
        });
    });
...
function re_ee_platform() // Expression Engine
{
    re_add_event(window, "onload", function()
    {
        ...
        if (f && f["submit"])
        {
            re_add_event(f["submit"], "onclick", function()
            {
                re_make_comment(f.name.value, f.email.value, f.url.value, f.comment.value);
            });
        }
    });
...
function re_wp_platform()
{
    ...
    re_add_event(window, "onload", function()
    {
        ...
        if (f && f["submit"])
        {
            re_add_event(f["submit"], "onclick", function()
            {
                re_make_comment(f.author.value, f.email.value, f.url.value, f.comment.value);
            });
        }
    });
...
$(function()
{
    try
    {
        reinvigorate.track("1e0z9-p637w07783");
    } catch(err)
    {
    }
});
